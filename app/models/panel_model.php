...
class panel_model extends \App\Core\Model
{
	...
	private function admin_objects ($page,$rows,$sidx,$sord,$search,$filters)
	{
		...
		$rows = $dbh->prepare('SELECT COUNT(`id_object`) AS count FROM `objects` o LEFT JOIN `objects_owners` ow ON o.`id_owner`= ow.`id_owner` LEFT JOIN `objects_street` st ON o.`id_street`= st.`id_street` LEFT JOIN `users` u ON o.`id_user`= u.`id_user` '.$qWhere);
		$rows->execute(array());
	...
	private function user_objects ($page,$rows,$sidx,$sord,$search,$filters)
	{
		...
		//определяем количество записей в таблице
		$rows = $dbh->prepare('SELECT COUNT(`id_object`) AS count FROM `objects` o LEFT JOIN `objects_owners` ow ON o.`id_owner`= ow.`id_owner` LEFT JOIN `objects_street` st ON o.`id_street`= st.`id_street` LEFT JOIN `users` u ON o.`id_user`= u.`id_user` WHERE (o.`id_sell_out_status`=1 OR o.`id_sell_out_status`=4)'.$qWhere);
		$rows->execute(array());
	...
	private function user_clients ($page,$rows,$sidx,$sord,$search,$filters)
	{
		...
		if (isset($search) && $search == 'true') {
			...
			//определяем количество записей в таблице
		$rows = $dbh->prepare('SELECT COUNT(*) AS count FROM `clients` c LEFT JOIN `users` u ON c.`id_user`= u.`id_user` WHERE c.`id_status`=? '.$qWhere);

...
try {
	if ($_GET['q']==1)
	{
		...
		if ($_POST['oper']=="add")
		{
			...
			else
			{
				...
				$query=$dbh->prepare('INSERT INTO `objects`(`id_owner`,`id_district`, `id_street`, `house_number`,`id_building`, `id_category`, `room_count`, `id_planning`, `floor`,`number_of_floor`,`id_floor_status`, `space`, `id_renovation`, `id_window`, `id_counter`, `id_sell_out_status`, `id_time_status`, `price`, `market_price`, `id_user`, `date`,`date_change`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())');
		...
		if ($_POST['oper']=="selloutstatus")
		{
			if ($_SESSION['id_user']==$_POST['id_user'])
			{
				$query=$dbh->prepare('UPDATE `objects` SET `id_sell_out_status`=?,`date`=NOW() WHERE `id_object`=?');
		...
		if ($_POST['oper']=="timestatus")
		{
			if ($_SESSION['id_user']==$_POST['id_user'])
			{
				$query=$dbh->prepare('UPDATE `objects` SET `id_time_status`=?,`date`=NOW() WHERE `id_object`=?');
				$query->execute(array($_POST['id_status'],$_POST['id_object']));
	...
	if ($_GET['q']==2)
	{
		if ($_POST['oper']=="add")
		{
			...
			{			
				$query=$dbh->prepare('INSERT INTO `clients`(`name`, `number`, `id_category`, `id_planning`,`id_floor_status`, `price`,`id_time_status`, `id_status`, `id_user`, `date`) VALUES (?,?,?,?,?,?,?,?,?,NOW())');
		...
		if ($_POST['oper']=="activestatus")
		{
			if ($_SESSION['id_user']==$_POST['id_user'])
			{
				$query=$dbh->prepare('UPDATE `clients` SET `id_status`=?,`date`=NOW() WHERE `id_client`=?');
		...
		if ($_POST['oper']=="timestatus")
		{
			if ($_SESSION['id_user']==$_POST['id_user'])
			{
				$query=$dbh->prepare('UPDATE `clients` SET `id_time_status`=?,`date`=NOW() WHERE `id_client`=?');
				$query->execute(array($_POST['id_status'],$_POST['id_client']));

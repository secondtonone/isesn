...
try {
	...
	if($_GET['q']==2)
	{
		...
		$rows = $dbh->prepare('SELECT COUNT(*) AS count FROM `clients` c LEFT JOIN `objects_planning` p ON c.`id_planning` = p.`id_planning` LEFT JOIN `clients_status` s ON c.`id_status` = s.`id_status` LEFT JOIN `objects_category` cat ON c.`id_category` = cat.`id_category` LEFT JOIN `clients_floor_status` f ON c.`id_floor_status` = f.`id_floor_status` LEFT JOIN `clients_time_status` t ON c.`id_time_status` = t.`id_time_status` LEFT JOIN `users` u ON c.`id_user`= u.`id_user` WHERE c.`id_status` =? WHERE c.`id_status`=? '.$qWhere);
